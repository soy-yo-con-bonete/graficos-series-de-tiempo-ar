    ...
    methods: {
        downloadData: function () {
            if (this.seriesArrayMeta.length === 0) {
                ...
                axios.get('http://apis.datos.gob.ar/series/api/series/', {
                    ...
                    console.log(error)
                    self.$toast('Error descargando datos de series, intente más tardde', {
                        className: 'et-alert',
        ...
        applySeriesTransformationsSingleSeries: function (seriesId) {
            ...
            for (let dateInd = 0; dateInd < self.rawData.transformed_data.length; dateInd++) {
                ...
                } else if (thisRepresentationMode.type === 'value_with_deflactor') {
                    let today = self.rawData.original_data[dateInd][0]
                    let deflactor = self.getDeflactorForDate(today)
                    newValue = newValue / deflactor
                }
        ...
        updateMetaArrayWithTransformations: function () {
            this.seriesArrayMeta = this.seriesArrayMeta.map(function (thisSeriesMeta, index) {
                ...
                if (thisRepresentationMode.type === 'index_100') {
                    ...
                    newSeriesMeta.serie_descripcion_transformada = newSeriesMeta.serie_descripcion + ' (Deflactado Dic-2016=1)'
                } else {
        ...
        getPriceDeflactorSeries: function () {
            ...
            axios.get(globalStore.metadataApiBaseUrl + 'deflactor/mensual').then(function (response) {
                ...
                self.deflactorIpc = {
                    data: data,
                    dates_in_ms: _.map(data, (thisDayData) => { return (new Date(thisDayData[0])).getTime() })
                }
            }).catch(function (error) {
                console.log(error)
                self.$toast('Error descargando serie IPC', {
                    className: 'et-alert',
                    horizontalPosition: 'center',
                    closable: 'true'
                })
            }).then(function () {
                self.$emit('price-deflactor-finished-loading')
            })
        },
        ...
        getDeflactorForDate: function (dateString) {
            let dateInMs = (new Date(dateString)).getTime()
            let index = this.deflactorIpc.dates_in_ms.indexOf(dateInMs)
            if (index >= 0) {
                return this.deflactorIpc.data[index][1] / 100
            } else {
                return null
            }
        }
